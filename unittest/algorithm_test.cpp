...
TEST_CASE("map2") {
  ...
  auto t3 =
      map([](auto x, auto y) -> decltype(x + y) { return x + y; }, t1, t2);
...
TEST_CASE("left_fold") {
  ...
  auto x = left_fold([](auto lhs, auto rhs) { return lhs * rhs; }, 1.0, t1);
  CHECK(x == 15);
...
TEST_CASE("find_if") {
  ...
  auto p1 =
      find_if([](auto x) { return x == htl::integral_constant<int, 1>(); }, t1);
  auto p2 = find_if(
      [](auto x) { return x == htl::integral_constant<int, 10>(); }, t1);
  auto p3 =
      find_if([](auto x) { return x == htl::integral_constant<int, 3>(); }, t1);
  auto p4 =
      find_if([](auto x) { return x == htl::integral_constant<int, 4>(); }, t1);
